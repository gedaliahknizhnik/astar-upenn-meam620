...
% Proportional/Derivative Gains for Translational Controller
kpxy = 80; kdxy = 2*sqrt(kpxy);
kpz = 80; kdz = 2*sqrt(kpz);

params.kp = diag([kpxy,kpxy,kpz]);
params.kd = diag([kdxy,kdxy,kdz]);

% Proportional/Derivative Gains for Orientation Controller
krthph = 8000; kwthph = 2*sqrt(krthph);
krpsi  = 1000; kwpsi  = 2*sqrt(krpsi);

params.kr = diag([krthph, krthph, krpsi]);
params.kw = diag([kwthph, kwthph, kwpsi]);
